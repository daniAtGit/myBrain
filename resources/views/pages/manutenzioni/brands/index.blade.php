...
{{--                    <h6 class="card-subtitle mb-2 text-body-secondary">Card subtitle</h6>--}}
                <div class="row">
                    ...
                    <div class="col-4 text-end">
                        <form method="post" action="{{route('manutBrands.store')}}">
                            @csrf
            ...
            <div class="row mt-3">
                ...
                <div class="col-10">
                    <table class="table table-hover table-bordered border" id="tabella">
                        ...
                        <tbody>
                            @foreach($manutBrands as $i => $manutBrand)
                                <tr>
                                    <td>{{$manutBrand->nome}}</td>
                                    <td>{{$manutBrand->oggetti->count()}}</td>
                                    <td>
                                        <button type="button" class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#edit{{$i}}">
                                            <i class="fa-solid fa-edit"></i>
                                        </button>
                                        @if(!$manutBrand->oggetti->count())
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h1 class="modal-title fs-5" id="exampleModalLabel">Modifica brand</h1>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <form action="{{route('manutBrands.update',$manutBrand)}}" method="post">
                            @csrf
                            @method('PATCH')

                            <div class="modal-body">
                                <input type="text" class="form-control" name="nome" placeholder="Nuovo" required value="{{$manutBrand->nome}}">
                            </div>
            ...
            <div class="modal fade" id="del{{$i}}" tabindex="-1" aria-labelledby="del{{$i}}" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <form action="{{route('manutBrands.destroy',$manutBrand)}}" method="post">
                            @csrf
                            @method('DELETE')

                            <div class="modal-body">
                                Vuoi davvero eliminare questo brand?
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-sm btn-light" data-bs-dismiss="modal">Chiudi</button>
